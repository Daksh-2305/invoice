...
foreach ($entry in $densities.GetEnumerator()) {
    ...
    foreach ($szDP in @(48, 108)) {
        ...
        # Scaling adjustment: 0.8 / 1.0
        # If adaptive (108), shrink it to 80% to fit 'safe zone'
        $finalScale = if ($szDP -eq 108) { 0.8 } else { 0.9 }
        $drawSz = [int]($px * $finalScale)
        $offset = [int](($px - $drawSz) / 2)
        
        $g.DrawImage($logo, $offset, $offset, $drawSz, $drawSz)
        $g.Dispose()
...
$logo.Dispose()
Write-Host "Done! All Android icons updated with the exact user-provided file (80% scaled for visibility)."
